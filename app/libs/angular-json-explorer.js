...
.directive('jsonExplorer', ['$http', function ($http) {
	...
	return {
		...
		link: function (scope, elem, attrs) {
			...
			//console.log("Started",scope, elem, attrs);
			...
			var parseRaw = function (k, v) {
				...
				if (typeof v == 'string') {
					// Date?
					var dateRegex = new RegExp('[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{0,4}Z', 'gmi');
					if (dateRegex.test(v)) {
						var date		= new Date(v);
						var nformat	= function(n) {
							if (n<10) {
								n = '0'+n;
							}
							return n;
						};
						var dateString	= nformat(date.getDate())+'/'+nformat(date.getMonth()+1)+'/'+nformat(date.getFullYear())+' at '+nformat(date.getHours())+':'+nformat(date.getMinutes())+':'+nformat(date.getSeconds())+'.'+nformat(date.getMilliseconds());
						
						return key + '<span class="string">"' + dateString + '"</span>';
					} else {
